...
# -*- coding: utf-8 -*-
"""
Photo Processor Pro — улучшенная, исправленная и полностью рабочая версия.
Поддерживает:
- CLI и опциональный Streamlit UI
- Несколько пресетов (JSON) или одиночный пресет через флаги CLI
- Обработку (удаление фона, попытка удаления водяного знака, изменение размера)
- Параллельную обработку с ThreadPoolExecutor (надёжнее для разных окружений)
- Сохранение результатов в подпапки по метке пресета
- Архивацию результатов во внешний ZIP (в /tmp или аналог)
...
try:
    ...
    raise SystemExit("OpenCV (cv2) обязателен. Установите: pip install opencv-python") from e

# rembg — опционально (лучше качество удаления фона при наличии)
try:
...
# --- ЛОГИРОВАНИЕ ---
LOG_FN = f"log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler(LOG_FN, encoding="utf-8"), logging.StreamHandler()],
)
...
# --- Класс пресета параметров обработки ---
@dataclass
class Preset:
    label: str = "default"         # метка/подпапка для результатов этого пресета
    remove_bg: bool = True         # удалять ли фон
    remove_wm: bool = False        # удалять ли водяной знак (простая эвристика)
    wm_threshold: int = 220        # порог для бинаризации при поиске водяных знаков
    wm_radius: int = 5             # радиус inpaint
    fmt: str = "PNG"               # формат вывода: PNG или JPEG
    jpeg_q: int = 95               # качество JPEG
    ...
    def from_dict(cls, d: Dict[str, Any], label: Optional[str] = None) -> "Preset":
        """
        Создать Preset из словаря (используется при загрузке JSON пресетов).
        """
        return cls(
...
# --- Утилиты для файлов/путей ---
def validate_ext(p: Path) -> bool:
    """Проверка расширения файла по списку поддерживаемых."""
    return p.suffix.lower() in SUPPORTED_EXT

def get_image_files(inp: Path) -> List[Path]:
    """Получить упорядоченный список изображений в папке."""
    if not inp.exists() or not inp.is_dir():
        return []
    return [p for p in sorted(inp.iterdir()) if p.is_file() and validate_ext(p)]

def create_zip_of_output(output_dir: str, zip_name: Optional[str] = None) -> Path:
    """
    Создать ZIP-архив выходной папки во временной директории и вернуть его путь.
    Архив создаётся вне самой выходной директории.
    """
...
def rembg_background(pil_img: Image.Image) -> Image.Image:
    """Попытаться удалить фон с помощью rembg (если установлен)."""
...
def grabcut_background(pil_img: Image.Image) -> Image.Image:
    """
    Более общий fallback-метод: grabCut через OpenCV.
    Работает медленнее и менее стабильно, но не требует внешних зависимостей.
    """
    try:
...
def remove_background_pil(pil_img: Image.Image, use_rembg: bool) -> Image.Image:
    """Удалить фон: сначала rembg (если включён и доступен), иначе grabcut."""
...
def remove_watermark_cv(img_cv: np.ndarray, threshold: int = 220, radius: int = 5) -> np.ndarray:
    """
    Найти яркие области (по порогу), сделать морфологическую фильтрацию, затем inpaint.
    Простая эвристика, не заменяет специализированные алгоритмы.
    """
    try:
        bgr = img_cv[..., :3].copy()
...
def resize_image(img_cv: np.ndarray, target_width: Optional[int], target_height: Optional[int]) -> np.ndarray:
    """Изменить размер изображения с сохранением пропорций при указании одной стороны."""
    h, w = img_cv.shape[:2]
...
def save_image(img_cv: np.ndarray, out_path: Path, preset: Preset) -> bool:
    """
    Сохранить OpenCV-изображение в заданном формате.
    Поддерживает PNG (с альфой) и JPEG.
    """
    try:
...
# --- Функции обработки одного изображения ---
def process_bytes(name: str, data: bytes, preset: Preset, out_base: Path) -> str:
    """Обработать изображение, переданное в виде байтов (загруженные файлы)."""
    try:
        pil = Image.open(io.BytesIO(data)).convert("RGBA")
    except UnidentifiedImageError:
        return f"❌ {name}: не изображение или файл повреждён"
    except Exception as e:
        logger.exception("open uploaded failed")
        return f"❌ {name}: open failed: {e}"

    proc = remove_background_pil(pil, preset.remove_bg)
    if proc.mode != "RGBA":
        proc = proc.convert("RGBA")
    img_cv = cv2.cvtColor(np.array(proc), cv2.COLOR_RGBA2BGRA)
    if preset.remove_wm:
...
def process_disk(path: Path, preset: Preset, out_base: Path) -> str:
    """Обработать изображение, расположенное на диске (Path)."""
    try:
        pil = Image.open(path).convert("RGBA")
    except UnidentifiedImageError:
        return f"❌ {path.name}: не изображение или файл повреждён"
    except Exception as e:
...
def process_batch(
    ...
    """
    Сформировать задачи (для каждого файла × каждого пресета) и выполнить их параллельно.
    Результаты каждого пресета сохраняются в подпапке output_dir/<preset.label>.
    """
    out_base = Path(output_dir).expanduser().resolve()
    out_base.mkdir(parents=True, exist_ok=True)
    tasks: List[Tuple[str, Any, Preset]] = []

    # Если есть загруженные файлы — обрабатывать их, иначе — брать с диска
    if uploaded_files:
    ...
    # Параллельная обработка потоками
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for kind, payload, preset in tasks:
            if kind == "uploaded":
                name, data = payload
                futures.append(ex.submit(process_bytes, name, data, preset, out_base))
            else:
                path: Path = payload
                futures.append(ex.submit(process_disk, path, preset, out_base))
...
def load_presets(path: Path) -> List[Preset]:
    """
    Загрузить пресеты из JSON.
    Форматы:
      - [{"label": "...", ...}, ...]
      - {"presets": [ ... ]}
      - одиночный объект с параметрами
    """
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if isinstance(data, dict):
        arr = data.get("presets") if "presets" in data else [data]
...
def cli_main(argv=None):
    ...
    parser.add_argument("--input", "-i", default="./input", help="Папка с входными изображениями")
    parser.add_argument("--output", "-o", default="./output", help="Папка для результатов")
    parser.add_argument("--presets", help="JSON файл с пресетами (переопределяет CLI-флаги)")
    # одиночный пресет через флаги
    parser.add_argument("--label", default="default", help="Метка (папка) для одиночного пресета")
    parser.add_argument("--no-bg", dest="remove_bg", action="store_false", help="Отключить удаление фона")
    parser.add_argument("--remove-wm", dest="remove_wm", action="store_true", help="Включить удаление водяных знаков")
    ...
    parser.add_argument("--files", default=None, help="Список файлов в input через запятую (опционально)")
    ...
    # Попытка создать ZIP-архив с результатами
    try:
...
# --- Streamlit UI (упрощённый) ---
def run_streamlit():
    if not HAS_STREAMLIT:
        raise SystemExit("Streamlit не установлен. Установите через: pip install streamlit")
    ...
    use_uploaded = st.sidebar.checkbox("Use uploads instead of folder", value=True)
    presets_json = st.sidebar.text_area("Presets JSON (list or single)", value=json.dumps([{
        "label": "preview",
        "remove_bg": True,
        "remove_wm": False,
        "wm_threshold": 220,
    ...
    # Попытка распарсить пресеты
    try:
        parsed = json.loads(presets_json)
        tmpf = Path(tempfile.gettempdir()) / "tmp_presets.json"
        tmpf.write_text(json.dumps(parsed, ensure_ascii=False))
        presets = load_presets(tmpf)
    except Exception:
        st.sidebar.error("Invalid presets JSON; using default")
        presets = [Preset()]

    # Загрузка файлов через UI или выбор папки
    if use_uploaded:
        uploaded = st.file_uploader("Upload images", accept_multiple_files=True, type=[e.lstrip(".") for e in SUPPORTED_EXT])
    else:
        uploaded = None
    ...
    if st.button("Run"):
        ...
        if uploaded:
            ...
            logs = process_batch(None, output_dir, presets, uploaded_files=up_list)
        else:
            logs = process_batch(input_dir, output_dir, presets)
        for l in logs:
            if l.startswith("✅"):
                st.success(l)
            elif l.startswith("❌"):
...
# --- Точка входа ---
def main():
    # Режим запуска: если первый аргумент "streamlit" — запускаем UI, иначе CLI
