...
Поддерживает:
- выбор папок/файлов локально (multiselect + file_uploader);
- рекурсивный поиск;
- удаление фона (rembg если доступен, иначе простой фолбэк);
- инпейнтинг водяных знаков (OpenCV);
- прогресс, лог и предпросмотр.
Если streamlit отсутствует — доступен CLI-режим.
"""
...
def save_image(img_cv: np.ndarray, out_path: Path, fmt: str, jpeg_q: int=95):
...
def main_streamlit():
    ...
    if page == "Обзор":
        ...
        if REMBG_AVAILABLE:
            st.success("rembg доступен — удаление фона будет выполняться моделью.")
    elif page == "Настройки":
        ...
        st.markdown("— Или загрузить отдельные файлы для обработки —")
        uploaded = st.file_uploader("Загрузите файлы (можно несколько)", accept_multiple_files=True, type=list(x.strip(".") for x in IMG_EXTS))
        st.session_state.setdefault("selected_folders", selected)
        st.session_state.setdefault("uploaded", uploaded)
        st.success("Настройки сохранены в сессии. Перейдите в 'Обработка' для запуска.")
    elif page == "Обработка":
        st.header("Запуск обработки")
        # load settings from session or defaults
        root = st.session_state.get("root", str(Path.cwd()))
        folder_options = st.session_state.get("selected_folders", None)
        uploaded = st.session_state.get("uploaded", None)

        # For convenience, allow quick selection here too
        root = st.text_input("Корневая папка (локальная)", value=str(Path.cwd()))
        root_p = Path(root)
        folder_options = [str(p) for p in list_subfolders(root_p)]
        selected = st.multiselect("Папки для обработки", options=folder_options, default=[str(root_p)])
        recursive = st.checkbox("Рекурсивно", value=False)
        ...
        uploaded = st.file_uploader("Или загрузите отдельные файлы (опционально)", accept_multiple_files=True, type=list(x.strip(".") for x in IMG_EXTS))
        ...
        if st.button("🚀 Запустить"):
            ...
            # include uploaded in-memory files
            mem_files = uploaded or []
            if mem_files:
                # save uploaded to temp in-memory list of tuples (name, bytes)
                mem = []
                for uf in mem_files:
                    try:
                        b = uf.read()
                        temp = {"name": uf.name, "bytes": b}
                        mem.append(temp)
                    except Exception as e:
                        logger.error(f"Ошибка чтения загруженного файла {uf.name}: {e}")
                # display but they'll be processed in-memory below
            else:
                mem = []
            ...
            ok, msg = validate_paths(dirs[0] if dirs else Path.cwd(), Path(output_dir))
            if not ok:
                st.error(msg); return
            ...
            for p in images:
                try:
                    with Image.open(p) as pil:
                        ...
                        out_name = p.stem
                        out_path = save_image(img_cv, Path(output_dir) / out_name, fmt, jpeg_q)
                        msg = f"✅ {idx+1}/{total}: {p.name} → {out_path.name}"
                        logs.append(msg)
                        log_box.code("\n".join(logs[-8:]))
                        idx += 1
                except UnidentifiedImageError:
                    err = f"❌ {idx+1}/{total}: Не удалось открыть {p.name}"
                    logs.append(err); log_box.code("\n".join(logs[-8:])); logger.error(err); idx += 1
                except Exception as e:
                    err = f"❌ {idx+1}/{total}: Ошибка {p.name} — {e}"
                    logs.append(err); log_box.code("\n".join(logs[-8:])); logger.error(traceback.format_exc()); idx += 1
                progress.progress(idx / total)

            # process in-memory uploads
            for mf in mem:
                try:
                    ...
                    if remove_wm:
                        img_cv = remove_watermark_cv(img_cv, wm_threshold, wm_radius)
                    out_name = Path(mf["name"]).stem
                    out_path = save_image(img_cv, Path(output_dir) / out_name, fmt, jpeg_q)
                    msg = f"✅ {idx+1}/{total}: {mf['name']} → {out_path.name}"
            ...
            try:
                out_files = sorted(Path(output_dir).glob("*"))
                for f in out_files:
                    st.write(f.name)
            except Exception:
                pass
...
def process_cli(input_dirs: List[str], output_dir: str, recursive: bool,
    ...
    if not images:
        ...
        return
    ok, msg = validate_paths(dirs[0], Path(output_dir))
    if not ok:
    ...
    for idx, p in enumerate(images):
        try:
            with Image.open(p) as pil:
                ...
                if remove_wm:
                    img_cv = remove_watermark_cv(img_cv, wm_threshold, wm_radius)
                out_path = save_image(img_cv, Path(output_dir) / p.stem, fmt, jpeg_q)
                msg = f"✅ {idx+1}/{len(images)}: {p.name} → {out_path.name}"
