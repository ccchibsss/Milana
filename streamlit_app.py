...
class HighVolumeAutoPartsCatalog:
    ...
    def apply_markup(self, price: float, brand: str) -> float:
        """Применение наценки (по бренду или глобальной)"""
    ...
    def build_export_query(self, selected_columns: Optional[List[str]] = None, include_prices: bool = True, apply_markup: bool = True) -> str:
        ...
        standard_description = """Состояние товара: новый (в упаковке). Высококачественные автозапчасти и автотовары — надежное решение для вашего автомобиля. Обеспечьте безопасность, долговечность и высокую производительность вашего авто с помощью нашего широкого ассортимента оригинальных и совместимых автозапчастей. В нашем каталоге вы найдете тормозные системы, фильтры (масляные, воздушные, салонные), свечи зажигания, расходные материалы, автохимию, электрику, автомасла, инструмент, а также другие комплектующие, полностью соответствующие стандартам качества и безопасности. Мы гарантируем быструю доставку, выгодные цены и профессиональную консультацию для любого клиента — автолюбителя, специалиста или автосервиса. Выбирайте только лучшее — надежность и качество от ведущих производителей."""
    ...
    def _show_delete_by_artikul(self):
        ...
        st.info("🔍 Поиск по артикулу (без учёта регистра и спецсимволов)")
    ...
    def delete_by_brand(self, brand_norm: str) -> int:
        ...
        with self.conn.transaction():
            ...
            # Опционально: чистка оставшихся OE
            return deleted
    ...
    def perform_cloud_sync(self):
        ...
        with st.spinner("Выполняется синхронизация..."):
            try:
                # Здесь будет интеграция с boto3, google.cloud.storage и т.д.
    ...
    def show_export_interface(self):
        ...
        if st.button("🚀 Выполнить экспорт", type="primary"):
            ...
            with st.spinner("Формирование отчёта..."):
                ...
                # Здесь можно добавить export_to_excel и export_to_parquet
                else:
                    st.warning(f"Формат {export_format} ещё в разработке. Доступен только CSV.")
                    success = False
    ...
    def export_to_parquet(self, output_path: str, selected_columns: Optional[List[str]] = None, include_prices: bool = True, apply_markup: bool = True) -> bool:
        ...
        try:
            query = self.build_export_query(selected_columns, include_prices, apply_markup)
            df = self.conn.execute(query).pl()  # Уже используем Polars — идеально для Parquet

            # Сохранение напрямую через Polars
