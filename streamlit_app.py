...
class HighVolumeAutoPartsCatalog:
    ...
    def load_price_file(self, price_file_path: str):
        if not price_file_path:
            return
        try:
            df_price = pl.read_excel(price_file_path, engine='calamine')
        except Exception as e:
            st.error(f"Ошибка при чтении прайса: {e}")
            return
        # Предполагается, что файл содержит колонки: артикул, бренд, цена
        if all(col in df_price.columns for col in ['артикул', 'бренд', 'цена']):
            ...
            self.prices_df = df_price.select(['artikul_norm', 'brand_norm', 'price'])
            st.success("Прайс загружен.")
        else:
            st.warning("Прайс-файл должен содержать колонки: 'артикул', 'бренд', 'цена'.")

    def apply_markups(self):
        # Общая наценка
        markup = st.number_input("Установить общую наценку (%)", min_value=0.0, max_value=100.0, value=0.0)
        self.price_markup = 1 + markup / 100

        # Наценки по брендам
        st.write("Настройка наценки по брендам:")
        # Можно реализовать интерфейс добавления/редактирования цен по брендам
        # Например, в виде таблицы
        # Для упрощения — оставить как пустую
        # Или реализовать редактирование через словарь
        # Пример:
        # self.brand_markup = {'toyota': 1.1, 'honda': 1.2}
        # Или оставить по умолчанию 1.0
        # Для примера, сделаем так:
        # Пустой словарь по умолчанию
        # Можно расширить в интерфейсе

        # Применение наценки к ценам
        if not self.prices_df.is_empty():
            df_prices = self.prices_df
            # Общее markup
            df_prices = df_prices.with_columns(
                (pl.col('price') * self.price_markup).alias('price_with_markup')
            )
            # Наценка по брендам
            for brand, markup_value in self.brand_markup.items():
                df_prices = df_prices.with_columns(
                    pl.when(pl.col('brand_norm') == brand).then(pl.col('price') * markup_value).otherwise(pl.col('price_with_markup')).alias('price_with_markup')
                )
            # Обновляем цены в parts_data
            for row in df_prices.iter_rows():
                ...
                """, [price, artikul_norm, brand_norm])

    def add_price_from_file(self, price_file_bytes):
        # Обработка файла с ценами
        try:
            df_price = pl.read_excel(io.BytesIO(price_file_bytes), engine='calamine')
        except Exception as e:
    ...
    def set_brand_markup(self, brand: str, markup_percent: float):
        normalized_brand = self.normalize_key(pl.Series([brand]))[0]
        self.brand_markup[normalized_brand] = 1 + markup_percent / 100
        st.info(f"Наценка для бренда '{brand}' установлена: {markup_percent}%")
        self.apply_markups()

    def set_global_markup(self, markup_percent: float):
        self.price_markup = 1 + markup_percent / 100
        st.info(f"Общая наценка установлена: {markup_percent}%")
        self.apply_markups()

    def export_with_exclusions(self, exclude_keywords: List[str], selected_columns: List[str], format_type: str):
        # Экспорт с исключениями по названиям
        query = self.build_export_query(selected_columns)
        df = self.conn.execute(query).pl()

        # Исключения
        for keyword in exclude_keywords:
            df = df.filter(~pl.col('Наименование').str.contains(keyword, case=False))
        # Экспорт
        if format_type == 'csv':
            buf = io.StringIO()
    ...
    def add_category(self, search_name: str, category_name: str):
        # Поиск по похожим названиям и установка категории
        # Можно расширить алгоритм поиска по схожести
        # Для простоты — ищем точное совпадение и обновляем
        normalized_name = self.normalize_key(pl.Series([search_name]))[0]
        # Обновляем в базе все записи, где название похоже
        self.conn.execute("""
            UPDATE oe_data SET category = ? WHERE name ILIKE '%' || ? || '%'
        """, [category_name, search_name])
        st.info(f"Категория '{category_name}' установлена для похожих товаров на '{search_name}'.")
    ...
    # Основная функция для загрузки файла цен
    def load_price_file(self, price_file_bytes):
        try:
            df_price = pl.read_excel(io.BytesIO(price_file_bytes), engine='calamine')
        except Exception as e:
            st.error(f"Ошибка чтения прайса: {e}")
    ...
    def export_to_csv(self, output_path: str, selected_columns: List[str]):
        query = self.build_export_query(selected_columns)
        df = self.conn.execute(query).pl()
    ...
    def build_export_query(self, selected_columns: List[str]):
        # Создаем полный запрос, включая описание
...
Выбирайте только лучшее — надежность и качество от ведущих производителей."""
    ...
    def export_to_csv(self, selected_columns: List[str], output_path: str):
        query = self.build_export_query(selected_columns)
        df = self.conn.execute(query).pl()
    ...
    def export_to_excel(self, selected_columns: List[str], output_path: Path):
        total_records = self.get_total_records()
        if total_records == 0:
            st.warning("Нет данных для экспорта.")
            return
        num_files = (total_records + EXCEL_ROW_LIMIT - 1) // EXCEL_ROW_LIMIT
        progress_bar = st.progress(0, text="Подготовка к экспорту...")
        exported_files = []
        ...
        # Создание ZIP, если несколько файлов
    ...
    def export_to_parquet(self, selected_columns: List[str], output_path: str):
        query = self.build_export_query(selected_columns)
        df = self.conn.execute(query).pl()
    ...
    def delete_by_brand(self, brand_norm: str) -> int:
        try:
            ...
            # Удаление из cross_references не нужно тут
            return res.rowcount
        except Exception as e:
            logging.exception(e)
            return 0
...
def main():
    ...
    if menu == "Загрузка данных":
        ...
        option = st.radio("Действия", ["Удалить по бренду", "Удалить по артикулу"])
