...
class HighVolumeAutoPartsCatalog:
    ...
    def detect_columns(self, actual_columns: List[str], expected_columns: List[str]) -> Dict[str, str]:
        ...
        column_variants = {
            ...
            'dimensions_str': ['весогабариты', 'размеры', 'dimensions', 'size'], 'price': ['цена', 'стоимость', 'price'],
    ...
    def export_to_excel(self, output_path: Path, exclusions=None, columns=None) -> tuple[bool, Path | None]:
        total_records = self.conn.execute("SELECT count(*) FROM (SELECT DISTINCT artikul_norm, brand_norm FROM parts_data) AS t").fetchone()[0]
        if total_records == 0:
            st.warning("Нет данных для экспорта")
            return False, None
    ...
    def show_export_interface(self):
        ...
        # Allow user to exclude certain rows based on names or partial matches
        exclusions = st.text_area("Исключить строки по наименованию (разделяйте символами '|'):",
                                  placeholder="Пример: фильтр масляный|лампы накаливания")
    ...
    def show_management_interface(self):
        ...
        if st.button("Применить общую наценку ко всем брендам", type="primary"):
            # Apply global markup to all brands without individual settings
        ...
        if brand:
            ...
            current_markup = current_markup[0] if current_markup else 0.0
            brand_markup = st.number_input(f"Наценка для бренда '{brand}' (%)", min_value=0.0, max_value=100.0, value=float(current_markup), step=0.1)
            if st.button("Установить индивидуальную наценку", type="primary"):
...
def main():
    ...
    - **Инкрементальные обновления**:```python
            # Delete associated cross_references that no longer have matching parts_data
            self.conn.execute("DELETE FROM cross_references WHERE (artikul_norm, brand_norm) NOT IN (SELECT DISTINCT artikul_norm, brand_norm FROM parts_data)")
...
def main():
    ...
    if menu_option == "Загрузка данных":
        ...
        **Типы Файлов:**
        ...
        - **Цены**: Рекомендуемые цены по артикулам и брендам.```python
            # Delete associated cross_references that no longer have matching parts_data
            self.conn.execute("DELETE FROM cross_references WHERE (artikul_norm, brand_norm) NOT IN (SELECT DISTINCT artikul_norm, brand_norm FROM parts_data)")
...
def main():
    ...
    if menu_option == "Загрузка данных":
        ...
        **Типы Файлов:**
        ...
        - **Весогабариты**: Размеры и вес товаров.
        - **Изображения**: Ссылки на изображения.
        - **Цены**: Рекомендуемые цены по артикулам и брендам.
        """)
